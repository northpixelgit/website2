...
The search script lives inside this skill's own directory. Invoke it by the path below, which is relative to the project root — run it from the project root, or prefix the path with the project root if the working directory is elsewhere:
